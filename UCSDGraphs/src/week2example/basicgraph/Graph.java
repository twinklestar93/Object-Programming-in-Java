...
public abstract class Graph {
	...
	public List<Integer> degreeSequence() {
		...
		// For each vertices in the graph, we need to find the inner and outer degrees
		for (int i = 0; i < numVertices; i++){
			degreeSequence.add(getNeighbors(i).size() + getInNeighbors(i).size());
		}
		// Since the degree we want is a non-decreasing list, we need to sort the degreeSequence out
		Collections.sort(degreeSequence, Collections.reverseOrder());
		System.out.println("\nArrayList in decreasing order.");
		for (Integer ele : degreeSequence){
	...
	public static void main (String[] args) {
		...
		GraphLoader.loadRoutes("data/airports/routesUA.dat", airportGraph1);
		System.out.println(airportGraph1);
		System.out.println("Observe most degrees are small (1-30), eight are over 100.");
		System.out.println("****");
		GraphAdjMatrix airportGraph2 = new GraphAdjMatrix();
		GraphLoader.loadRoutes("data/airports/routesUA.dat", airportGraph2);
		System.out.println(airportGraph2);
		System.out.println("Observe most degrees are small (1-30), eight are over 100.");
		System.out.println("****");
		...
		System.out.println(airportGraph1.getDistance2(2));
		System.out.println(airportGraph2.getDistance2(2));
	}
}
