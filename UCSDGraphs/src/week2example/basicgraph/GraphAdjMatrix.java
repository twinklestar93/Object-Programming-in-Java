...
public class GraphAdjMatrix extends Graph {
	...
	public List<Integer> getDistance2(int v) {
		...
		// get the twoHopsMatrix
		int[][] twoHopsMatrix = power2Matrix(adjMatrix);
		int dim = twoHopsMatrix.length;
		for (int i = 0; i < dim; i++){
			for (int j = 0; j < twoHopsMatrix[v][i]; j++){
				twoHops.add(i);
			}
		}
		return twoHops;
	...
	/*A helper function that multiply two matrixes
	 * 
	 * @para a
	 * 		the first matrix
	 * @para b
	 * 		the second matrix
	 * return c
	 * 		the resultant matrix of matrix a multiplied by matrix b
	 * 
	 * */
	private int[][] multiply(int[][] a, int[][] b){
		// to check whether the matrix a and b satisfy the condition
		int rowA = a.length;
		int colA = a[0].length;
		int rowB = b.length;
		int colB = b[0].length;
	...
	/*A helper function that calculate the 2nd power of a matrix 
	 * 
	 * @para mat
	 * 
	 * return 
	 * 		result
	 * */
	private int[][] power2Matrix(int[][] mat){
		int[][] result = multiply(mat, mat);
		return result;
	}
}
