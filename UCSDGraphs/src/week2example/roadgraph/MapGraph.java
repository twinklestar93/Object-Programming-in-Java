...
public class MapGraph {
										  ...
										  GeographicPoint goal, Consumer<GeographicPoint> nodeSearched)
	{
		...
		while (!toExplore.isEmpty()) {
			...
			for (MapNode neighbor : neighbors) {
				if (!visited.contains(neighbor)) {
					for (MapEdge eg : next.getEdges()){
						if (eg.getEndNode().equals(neighbor)){
							neighbor.setActualDistance(next.getDistance() + eg.getLength());
							break;
						}
					}
		...
		System.out.println("Items removed in dijkstra's algorithm: " + itemsRemoved);
		// Reconstruct the parent path
		List<GeographicPoint> path =
				reconstructPath(parentMap, startNode, endNode);
											 ...
											 GeographicPoint goal, Consumer<GeographicPoint> nodeSearched)
	{
		...
		while (!toExplore.isEmpty()) {
			...
			for (MapNode neighbor : neighbors) {
				if (!visited.contains(neighbor)) {
					...
					double straightDistance = neighbor.getLocation().distance(endNode.getLocation());
					// If path through curr to n is shorter
					if (neighbor.getActualDistance() + straightDistance < neighbor.getDistance()){
						// Update n's distance
						neighbor.setDistance(neighbor.getActualDistance() + straightDistance);
						// Update curr as n's parent in parent map
						if (parentMap.get(neighbor) != null){
							parentMap.remove(neighbor);
						}
		...
		System.out.println("Items removed in aStarSearch algorithm: " + itemsRemoved);
		// Reconstruct the parent path
	...
	public static void main(String[] args)
	{
		...
		*/ 
		...
		List<GeographicPoint> route1 = theMap1.dijkstra(start, end);
